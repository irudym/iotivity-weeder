...
OCRepresentation TemperatureSensor::getRepresentation() {
    if(m_MRAA == NULL) {
        //throw exception
        return m_Representation;;
    }
    float temp = m_MRAA->getTemperatureInC(m_Pin);
    m_Representation.setValue(TEMPERATURE_RESOURCE_KEY, temp);
    return m_Representation;
}

void TemperatureSensor::observerLoop() {
    ...
    try {
        ...
        cout << "[TemperatureSensor]::Exception during notifying observers: " << e.what() << endl;
        //m_ObserverLoop->stop();
    }
}

OCEntityHandlerResult TemperatureSensor::entityHandler(shared_ptr<OCResourceRequest> Request) {
    OCEntityHandlerResult result = OC_EH_ERROR;
    //cout << "[TemperatureSensor]:: call for sensor: " << m_Name << endl;
    if(Request) {
